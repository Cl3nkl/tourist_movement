...
class Map extends Component {
  ...
  componentDidMount() {
    ...
    countOfDays = 10; ////////////////////////HERE////////////////set count of days
    //////////////////change stop interval in slider.js/45 
    maxOneDayDotsCountry = 1000////////////////////////HERE////////////////set max count of dots per country and day 
    
    const poly = new Polygon();// create poly object    
    
    // countOfDays = 5; //set date period

    for(let i = 0; i<countOfDays; i++){
      dotsPL = Math.floor((Math.random() * 500) + 1);
      dotsRU = Math.floor((Math.random() * 500) + 1);
      dotsDE = Math.floor((Math.random() * 500) + 1);
      dotsEN = Math.floor((Math.random() * 500) + 1);
      dotsCounts[i] = [dotsPL,dotsRU,dotsDE,dotsEN];
      randomPoints[i] = poly.generate(dotsPL,dotsRU,dotsDE,dotsEN);
    ...
    this.layer = (day = days[0],short = 'default',color = 'gray')=>{
        ...
        if(map.getSource(daysString[day])){
          if(short == 'default'){
            ...
            // document.body.addEventListener("TOGGLE_POSITION",(e)=>{
            ...
            //document.getElementById('autoPlay').innerHTML = countryName[short]+'('+dotsCounts[days[day]][countryNr[short]]+')';
        ...
        this.geoJsonData = (day = 0) =>{
          ...
          if(loader == true){
            ...
            helpDay = day;
              ...
              checkLangLayers();
              checkSource();
              sources(randomPoints[helpDay],daysString[helpDay]);
